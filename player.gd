...
func _ready() -> void:
	...
	gravity_scale = 0.0   # planets supply gravity via apply_central_force
	collision_layer = 1
	collision_mask  = 3   # layer 1 (ships) + layer 2 (planets)
...
func get_input() -> void:
	...
	if Input.is_action_pressed("thrust") and has_fuel():
		...
		change_state(IDLE)
		set_collision_mask_value(2, true)   # stop at planet surface when coasting
...
func update_fuel(delta: float) -> void:
	if thrust.length() > 0.0:
		...
		if food_amount > 0.0:
			var used := min(food_amount, delta)
			food_amount -= used
